...
static inline void sha1_update_xor(Sha1 *sha1, uint8_t *data, size_t data_length, uint8_t xor_value);
...
void hmac_sha1(HmacSha1 *scratchpad, const uint8_t *key, size_t key_length, const uint8_t *message, size_t message_length, uint8_t *code_output)
{
    ...
    sha1_init(&scratchpad->sha1);
    sha1_update_xor(&scratchpad->sha1, scratchpad->key_block, SHA1_BLOCK_BYTES, HMAC_INNER_PAD_BYTE);
    ...
    sha1_init(&scratchpad->sha1);
    sha1_update_xor(&scratchpad->sha1, scratchpad->key_block, SHA1_BLOCK_BYTES, HMAC_OUTER_PAD_BYTE);
    sha1_update(&scratchpad->sha1, scratchpad->inner_digest, SHA1_DIGEST_BYTES);
    sha1_digest(&scratchpad->sha1, code_output);
}

static inline void sha1_update_xor(Sha1 *sha1, uint8_t *data, size_t length, uint8_t xor_value)
{
    // Modify data in-place to hold the xored version
    for (size_t i = 0; i < length; i++) {
        data[i] ^= xor_value;
    }
    // Update sha1 with xored version
